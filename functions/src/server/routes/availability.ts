...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const conditions = [
      eq(availability.service, service as string),
      eq(availability.date, date as string),
    ];

    // Add slot condition if provided
    if (slot) {
      conditions.push(eq(availability.slot, slot as string));
    } else {
    ...
    if (!availabilityRecord || availabilityRecord.length === 0) {
      const defaultCapacity = getCapacityForService(service as string);
