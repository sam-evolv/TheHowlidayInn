...
function isLikelySender(v) {
    return !!v && /^\d{6,}$/.test(v);
}
function isLikelyAppId(v) {
    return !!v && /^1:\d+:web:[a-f0-9]+$/i.test(v);
}
function isLikelyMeasurementId(v) {
    return !v || /^G-[A-Z0-9]+$/.test(v);
}
function looksLikeBucket(v) {
...
function looksLikeAuthDomain(v) {
    return !!v && (v.endsWith(".firebaseapp.com") || v.includes("."));
}
export function checkFirebaseEnv(env) {
    const report = {
        ok: true,
        missing: [],
        warnings: [],
...
export function firebaseHealthHandler(req, res) {
    const report = checkFirebaseEnv(process.env);
    return res.json({
        ok: report.ok,
