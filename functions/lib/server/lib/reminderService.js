...
export async function sendBookingReminder(bookingId) {
    try {
        ...
        const alreadySent = await db.query.reminders.findFirst({
            where: and(eq(reminders.bookingId, bookingId), eq(reminders.reminderType, 'day_before'), eq(reminders.status, 'sent'))
        });
        if (alreadySent) {
...
export async function checkAndSendReminders() {
    try {
        ...
        const upcomingBookings = await db
            .select()
            .from(bookings)
            .where(and(gte(bookings.startDate, startOfTargetDay), lte(bookings.startDate, endOfTargetDay), sql `${bookings.status} IN ('pending', 'confirmed', 'paid')`));
        ...
        for (const booking of upcomingBookings) {
            ...
            if (success) {
                ...
                const alreadySent = await db.query.reminders.findFirst({
                    where: and(eq(reminders.bookingId, booking.id), eq(reminders.reminderType, 'day_before'), eq(reminders.status, 'sent'))
                });
                if (alreadySent) {
                    skipped++;
                }
