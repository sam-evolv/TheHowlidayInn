...
export function getCapacityForService(service) {
    const normalized = service.toLowerCase();
    if (normalized === 'daycare')
        return capacityConfig.maxCapacityDaycare;
    if (normalized === 'boarding')
        return capacityConfig.maxCapacityBoarding;
    if (normalized === 'trial day' || normalized === 'trial')
        return capacityConfig.maxCapacityTrial;
    // Default fallback
    return capacityConfig.maxCapacityDaycare;
}
