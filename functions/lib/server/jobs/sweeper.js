...
export async function sweepExpiredReservations() {
    try {
        ...
        const expiredReservations = await db
            .select()
            .from(reservations)
            .where(and(eq(reservations.status, "active"), lt(reservations.expiresAt, now)));
...
export function startSweeper(intervalMinutes = 1) {
    if (sweeperInterval) {
...
// Alias for Firebase Functions scheduled job
export const expireStaleReservations = sweepExpiredReservations;
