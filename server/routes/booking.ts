...
bookingRouter.post("/api/checkout/create-intent", requireAuth, async (req: any, res) => {
  try {
...
bookingRouter.post("/api/payments/create-intent", requireAuth, async (req: any, res) => {
  try {
    const { amount, currency = "eur", customerId } = req.body || {};
...
async function checkCapacityAvailability(serviceType: string, startDate: Date, endDate: Date) {
  try {
    ...
    const capacity = settings.capacity as any;
...
bookingRouter.get("/api/booking/capacity", async (req: any, res) => {
  try {
...
// Validate before pay
bookingRouter.get("/api/booking/validate", requireAuth, async (req: any, res) => {
  const { dogId, start, end } = req.query as any;
  if (!dogId || !start || !end) {
  ...
  if (!sets || !sets.requiredVaccines || (sets.requiredVaccines as any[]).length === 0) {
...
bookingRouter.post("/api/checkout/create-intent", requireAuth, async (req: any, res) => {
  ...
  // Using canonical service keys with colons
  ...
  if (!basePrice) {
    console.error('[checkout/create-intent] Invalid service type received:', serviceType);
    return res.status(400).json({ error: `Invalid service type: ${serviceType}` });
  }
