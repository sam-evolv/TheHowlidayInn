...
router.post("/", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Check if facility is closed on selected date
    if (isClosed(dateObj)) {
      return res.status(400).json({
        success: false,
        error: "The facility is not available on the selected date. Please choose a different date."
      });
    }

    // Check if trial day is on weekend (Trial days are Mon-Fri only)
    if (service === "Trial Day" && isWeekend(dateObj)) {
      return res.status(400).json({
        success: false,
        error: "Trial days are only available Monday through Friday."
      });
    }

    // Validate time slot against daily windows
    if (slot && slot !== "ALL_DAY") {
      const validWindows = getDailyWindows(dateObj);
      const validSlots = validWindows.map(w => w.label || `${w.start}-${w.end}`);
      
      if (!validSlots.includes(slot)) {
        return res.status(400).json({
          success: false,
          error: `Invalid time slot for selected date. Valid slots: ${validSlots.join(", ")}`
        });
      }
    }
    ...
    const result = await db.transaction(async (tx: PgTransaction<NodePgQueryResultHKT, Record<string, never>, any>) => {
      ...
      conditions.push(eq(availability.slot, slotValue));
